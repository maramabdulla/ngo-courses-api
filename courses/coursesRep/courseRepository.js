...
function getOneCourse(id,callback){
    let sql = ` select e.id  , e.title  , e.description ,e.start_date , e.location,e.number_of_seats,e.trainer,a.name  from courses e   join trainers a on e.trainer=a.id join ngos o on e.id_ngo=o.id  where e.id= `+id+` `;
    createDatabaseConnection((connectError, connection) => {
        if (connectError) {
            callback(connectError, null);
        } else {
